...
def get_current_language():
    """Get the current language from session or detect from browser."""
    ...
    # Default to Arabic if no match or if match is not in supported list
    return best_match if best_match else DEFAULT_LANGUAGE
...
def init_database():
    ...
    # Migration: add new metadata columns if missing
...
def add_book():
    ...
    if request.method == 'POST':
        ...
        # Save book info to database (with cover image)
            conn = get_db_connection()
            conn.execute(
                'INSERT INTO books (title, author, description, filename, image_filename, discipline, isbn, publisher, page_count, language) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                (title, author, description, filename, cover_filename_to_save, discipline, 
                 request.form.get('isbn', ''), 
                 request.form.get('publisher', ''), 
                 int(request.form.get('page_count') or 0), 
...
def bookshelf():
    ...
    conn = get_db_connection()
    user_email = current_user.email if current_user.is_authenticated else session.get('email')
    
    books = conn.execute('''
        SELECT b.*, ub.added_at 
        FROM books b 
    ...
    return render_template('bookshelf.html', books=books, t=get_translations(), lang_data=get_language_data())

@app.route('/bookshelf/add/<int:book_id>', methods=['POST'])
@login_required
def add_to_bookshelf(book_id):
    ...
    exists = conn.execute('SELECT 1 FROM user_bookshelf WHERE user_email = ? AND book_id = ?', (user_email, book_id)).fetchone()
    
    if not exists:
        conn.execute('INSERT INTO user_bookshelf (user_email, book_id) VALUES (?, ?)', (user_email, book_id))
        conn.commit()
        flash('Book added to your bookshelf!', 'success')
    else:
        flash('Book is already in your bookshelf.', 'info')
        
    conn.close()
    return redirect(request.referrer or url_for('book_detail', book_id=book_id))

@app.route('/bookshelf/remove/<int:book_id>', methods=['POST'])
@login_required
def remove_from_bookshelf(book_id):
    ...
    conn.execute('DELETE FROM user_bookshelf WHERE user_email = ? AND book_id = ?', (user_email, book_id))
    conn.commit()
    conn.close()
    
    flash('Book removed from your bookshelf.', 'success')
...
def chat_api():
    """API endpoint for AI chat with context."""
    data = request.get_json()
    ...
    # Get response from AI using the full history
    response_text = get_ai_chat_response(session['chat_history'])
...
def get_ai_chat_response(messages_history):
    """
    Get response from AI via DeepSeek using conversation history.
    """
    api_key = os.getenv('DEEPSEEK_API_KEY')
    
    if not api_key:
        return "Configuration Error: DEEPSEEK_API_KEY is missing. Please add it to your .env file."
    ...
    system_prompt = {
        "role": "system", 
        "content": """You are an intelligent and helpful AI assistant for a digital library application. 
        Your goal is to provide accurate, detailed, and relevant answers to the user's questions.
        
        Guidelines:
        1. Context Awareness: Always consider the previous messages in the conversation to provide relevant follow-up answers.
        2. Accuracy: Ensure your information is correct and up-to-date. If you are unsure, admit it politely.
        3. Detail: Provide comprehensive answers. Avoid one-word responses unless specifically asked.
        4. Tone: Maintain a professional, encouraging, and friendly tone.
        5. Language: Respond in the same language as the user's message (English or Arabic).
        
        If the user asks about the library's features, you can mention:
        - Browsing books by category.
        - Searching for books using keywords or AI semantic search.
        - Reading books online or downloading them.
        - Creating a personal bookshelf.
        """
    }
    
    # Construct the full message list for the API
    # Start with system prompt, then add history
    ...
    try:
        ...
        # Check for API errors even if status is 200
        if 'error' in data:
        ...
        if 'choices' in data and len(data['choices']) > 0:
            ...
            return "I didn't get a valid response from the AI service."
...
def chunk_text(text, max_chars=15000):
    """
    Split text into chunks to respect API token limits.
    Using character count as a rough proxy for tokens (approx 4 chars per token).
    15000 chars is roughly 3750 tokens, leaving room for response.
    """
    chunks = []
    current_chunk = ""
    
    # Simple split by paragraphs to avoid breaking sentences
...
def summarize_book():
    """
    API endpoint to summarize book text using DeepSeek.
    Payload: { "text": "book content..." }
    """
    try:
        data = request.get_json()
        text = data.get('text', '')
        ...
        # Process chunks (limit to first 3 chunks to avoid excessive API usage/time)
        # In a production app, you might want to summarize all chunks and then summarize the summaries
        process_chunks = chunks[:3] 
        
        for i, chunk in enumerate(process_chunks):
            messages = [
                {"role": "system", "content": "You are a helpful assistant that summarizes books. Provide a concise summary of the provided text."},
                {"role": "user", "content": f"Summarize this section of the book:\n\n{chunk}"}
            ]
            
            try:
                result = call_deepseek_api(messages, model="deepseek-chat")
        ...
        # If we have multiple chunks, do a final pass to unify the summary
        if len(summaries) > 1:
            final_messages = [
                {"role": "system", "content": "You are a helpful assistant that summarizes books. Combine the following section summaries into one coherent book summary."},
                {"role": "user", "content": final_summary_text}
            ]
            try:
                final_result = call_deepseek_api(final_messages, model="deepseek-chat")
                if 'choices' in final_result and len(final_result['choices']) > 0:
                    final_summary_text = final_result['choices'][0]['message']['content']
            except Exception as e:
...
def annotate_book():
    """
    API endpoint to generate annotations for book text.
    Payload: { "text": "book content..." }
    """
    try:
        data = request.get_json()
        ...
        # For annotations, we'll process the first substantial chunk to give a flavor
        # or we could process specific selected text if the UI supported it.
        # Here we'll take the first 10000 chars.
        process_text = text[:10000]
        
        messages = [
            {"role": "system", "content": "You are a helpful assistant that provides literary annotations. Identify key themes, character developments, and stylistic devices in the text. Return the result as a JSON list of objects with 'quote' and 'annotation' fields if possible, or just a structured text list."},
            {"role": "user", "content": f"Analyze and annotate the following text:\n\n{process_text}"}
        ]
        
        result = call_deepseek_api(messages, model="deepseek-chat")
        
        if 'choices' in result and len(result['choices']) > 0:
            annotations = result['choices'][0]['message']['content']
            return jsonify({
                'success': True,
